...
class MainTest extends AnyFunSuite {
  // Integration tests
  def getScript(inputs: List[String], startSquaresByPlayerNames: Map[String, Int]): List[String] = {
    val transcript = ListBuffer[String]()
    inputs.foldLeft(startSquaresByPlayerNames)((squaresByPlayerNames, input) => {
      Main.processInput(squaresByPlayerNames, input) match {
        case Left(error) =>
          transcript.append(error)
          squaresByPlayerNames
        case Right(newGameState, message) =>
  ...
  test("Adds a player to the game") {
    val squaresByPlayerNames = Map[String, Int]()
    val inputs = List("add player Pippo")
    val refOutputs = List("players: Pippo")
    assert(getScript(inputs, squaresByPlayerNames) == refOutputs)
  }
  ...
  test("Players can move by specifying dice") {
    val squaresByPlayerNames = Map("Pippo" -> 0, "Pluto" -> 0)
  ...
  test("Dice cannot be specified outside range 1-6") {
    val squaresByPlayerNames = Map("Pippo" -> 0)
    val inputs = List("move Pippo 4, 8")
    val List(output) = getScript(inputs, squaresByPlayerNames)
    assert(output.contains("Dice must have"))
  }

  test("Player wins if he or she lands on square 63") {
    val squaresByPlayerNames = Map("Pippo" -> 60)
  ...
  test("Game rolls dice automatically") {
    val squaresByPlayerNames = Map("Pippo" -> 4)
  ...
  test("Player doubles move if he or she lands on The Goose") {
    val squaresByPlayerNames = Map("Pippo" -> 3)
    val inputs = List("move Pippo 1, 1")
  ...
  test("Squares 5, 9, 14, 18, 23, 27 are all Goose squares") {
    ...
    List(5, 9, 14, 18, 23, 27).foreach(goose => {
      val squaresByPlayerNames = Map("Pippo" -> (goose - sum))
  ...
  test("Player can do multiple jumps from a Goose square") {
    val squaresByPlayerNames = Map("Pippo" -> 10)
  ...
  test("When one player lands on another's square, the latter is sent back to the first player's previous square") {
    val squaresByPlayerNames = Map("Pippo" -> 15, "Pluto" -> 17)
    val inputs = List("move Pippo 1, 1")
  ...
  test("Combine a prank and a bounce") {
    val squaresByPlayerNames = Map("Pippo" -> 62, "Pluto" -> 60)
  ...
  test("Combine a prank and a bridge") {
    val squaresByPlayerNames = Map("Pippo" -> 4, "Pluto" -> 12)
    val inputs = List("move Pippo 1, 1")
  ...
  test("Combine a prank and a single Goose jump (prank at end)") {
    val squaresByPlayerNames = Map("Pippo" -> 2, "Pluto" -> 26)
  ...
  test("Combine a prank and a double Goose jump (prank at end)") {
    val squaresByPlayerNames = Map("Pippo" -> 4, "Pluto" -> 19)
  ...
  test("Combine a prank and a double Goose jump (prank in middle)") {
    val squaresByPlayerNames = Map("Pippo" -> 4, "Pluto" -> 9)
    val inputs = List("move Pippo 2, 3")
